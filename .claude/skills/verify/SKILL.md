...
`POST /api/<opName>` with a JSON body, bearer token auth), node WS server on
...
unshare -r -n bash -c '
  ...
  # poll until up:
  curl -sf -X POST http://127.0.0.1:4141/api/getAuthState -H "Content-Type: application/json" -d null
  ...
'
```

- `SC_DATA_DIR` isolates all state (users, TLS, agents); `PORT` overrides 4141 only.
- `SC_BIND` sets the listen address and `SC_TRUSTED_PROXIES` (comma-separated) the
  proxies whose `X-Forwarded-For` is believed — both useful for probing the
  behind-a-proxy paths without a proxy (see `docs/reverse-proxy.md`).
...
```bash
# $API is http://127.0.0.1:4141/api — every op lives under the /api prefix.
