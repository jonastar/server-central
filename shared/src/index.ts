...
export type CentralApiOperations = {
    ...
    readHostComposeStacks: { data: { hostId: string }; response: HostComposeStacks };
    ...
    // Host dashboards — the per-host overview's widget arrangement.
    // `null` means this host has never been customized: the client builds the
    // default from its widget registry, which is also how a widget added in a
    // later release reaches every uncustomized host. See doc/idea_host_dashboard.md §3.
    getHostDashboard: { data: { hostId: string }; response: HostDashboard | null };
    setHostDashboard: { data: { hostId: string; widgets: DashboardWidgetInstance[] }; response: HostDashboard };
    // Drop the stored arrangement, returning the host to the registry default.
    resetHostDashboard: { data: { hostId: string }; response: void };
};
