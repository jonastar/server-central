                ...
                <section className="panel">
                    ...
                    {shown.length === 0 ? (
                        ...
                        <table className="data-table">
                            ...
                            <tbody>
                                {shown.map((u) => (
                                    <tr key={u.username} className={cx(u.uid === 0 && "row-status-warn")}>
                                        <td>
                                            <b>{u.username}</b>
                                            {u.uid === 0 && <span className="badge" style={{ marginLeft: 6 }}>root</span>}
                                        </td>
                                        <td className="dim">{u.uid}</td>
                                        <td className="dim">{u.groups.join(", ") || "—"}</td>
                                        <td className="mono dim">{u.home}</td>
                                        <td className="mono dim">{u.shell}</td>
                                        <td>
                                            {u.mappedBy.length > 0
                                                ? u.mappedBy.map((name) => <span key={name} className="badge badge-ok" style={{ marginRight: 4 }}>{name}</span>)
                    ...
                    <p className="dim" style={{ fontSize: 12, padding: "8px 12px" }}>
